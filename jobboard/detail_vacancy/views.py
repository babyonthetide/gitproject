...
class DetailVacancyView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["main_banner"] = (
            site_settings.main_banner if site_settings.main_banner else None
        )
        # Город пользователя
        context["user_city"] = get_object_or_404(
            Profile, user=self.request.user
        ).get_city_display()
        # Другие вакансии компании
        context["vacancies_company"] = Vacancy.objects.filter(
            company=vacancy.company
        ).exclude(id=vacancy.pk, hidden_by_user__user=self.request.user)
        ...
        context["experience_required"] = years_declension(
            vacancy.experience_required, vacancy.experience_from, vacancy.experience_to
        )
        ...
        context["user_favorites"] = FavoriteVacancy.objects.filter(
